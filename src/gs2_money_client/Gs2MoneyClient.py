...
class Gs2MoneyClient(AbstractGs2Client):
    ...
    def describe_receipt(self, request):
        ...
        """
        query_strings = {}
        if request.get_begin() is not None:
            query_strings['begin'] = request.get_begin()
        if request.get_end() is not None:
            query_strings['end'] = request.get_end()
        if request.get_page_token() is not None:
            query_strings['pageToken'] = request.get_page_token()
        if request.get_limit() is not None:
            query_strings['limit'] = request.get_limit()
    ...
    def charge_wallet_by_stamp_sheet(self, request):
        ...
        return ChargeWalletByStampSheetResult(self._do_post_request(
            url=Gs2Constant.ENDPOINT_HOST + "/wallet/" + str(("null" if request.get_slot() is None or request.get_slot() == "" else request.get_slot())) + "/stampSheet/charge",
            service=self.ENDPOINT,
    ...
    def consume_wallet_by_stamp_task(self, request):
        ...
        return ConsumeWalletByStampTaskResult(self._do_post_request(
            url=Gs2Constant.ENDPOINT_HOST + "/wallet/" + str(("null" if request.get_slot() is None or request.get_slot() == "" else request.get_slot())) + "/stampTask/consume",
            service=self.ENDPOINT,
    ...
    def describe_wallet(self, request):
        ...
        """
        query_strings = {}
        if request.get_user_id() is not None:
            query_strings['userId'] = request.get_user_id()
        if request.get_page_token() is not None:
            query_strings['pageToken'] = request.get_page_token()
        if request.get_limit() is not None:
            query_strings['limit'] = request.get_limit()
